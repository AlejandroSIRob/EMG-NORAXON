...
Ejemplo:
    python EMG_CSV.py "Pruebas\2025-12-03-17-36_C4.csv" --mvc-method peak --plot
"""
...
def analyze_emg_signal(
    ...
    # Normalización: Usar estimate_mvc para consistencia con EMG_CALIBRATE.py
...
def print_summary(results: Dict, file_name: str):
    ...
    print(f"  Método MVC: {meta.get('mvc_method', 'peak')}\n")
    ...
    print(f"  MVC ({env.get('mvc_method', 'peak')}): {env['mvc_value']:.3f} µV  (← Usar para normalización)\n")
...
Ejemplos de uso:
  ...
  python EMG_CSV.py "archivo.csv" --output ./results/ --mvc-method 95perc
  python EMG_CSV.py "archivo.csv" --mvc-method mean_top10 --plot
        """
    )
    ...
    parser.add_argument('--mvc-method', type=str, default='peak', choices=['peak', '95perc', 'mean_top10'], help='Método MVC: peak (máximo), 95perc (percentil 95), mean_top10 (media del 10% superior)')
