...
EJEMPLOS DE USO:
  # Modo simulación (sin servidor)
    python EMG.py --simulate --verbose

  # Conexión a servidor Noraxon real
    python EMG.py --ip localhost --port 9220 --endpoint /samples --verbose

  # Guardar RMS en CSV
    python EMG.py --simulate --save-csv output\\rms.csv --log-freq 5

  # Análisis completo con features y detección de fatiga
    python EMG.py --ip localhost --port 9220 --endpoint /samples `
    --save-csv output\rms.csv --save-features output\features.csv `
    --window-ms 200 --hop-ms 50 --verbose
            ...
            while True:
                ...
                if do_log:
                    for i, f in enumerate(forces):
                        logging.info("Chan %d: RMS=%.3f µV | Fuerza=%.2f%%", i, rms_list[i], f)
